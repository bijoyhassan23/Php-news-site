...
<?php
    if(isset($_GET['aut-id'])){
    include "config.php";
    $sql1 =  "SELECT * FROM user WHERE user_id = {$_GET['aut-id']}";
            ...
            <div class="col-md-8">
                ...
                <div class="post-container">
                  ...
                  <?php 
                        ...
                        <?php
                        $sqlPagQuery = "SELECT * FROM post WHERE post.author = '{$_GET['aut-id']}'";
                        ...
                        if(mysqli_num_rows($result2) > 0){
                            ...
                            if($curren_page > 1){
                                echo "<li><a href='{$_SERVER['PHP_SELF']}?aut-id={$_GET['aut-id']}&page=".($curren_page - 1)."'>Prev</a></li>";
                            }

                            for($i = 1; $i <= $pageCount; $i++){
                                $active_class = ($i == $curren_page) ? "active" : "";
                                echo "<li class='{$active_class}'><a href='{$_SERVER['PHP_SELF']}?aut-id={$_GET['aut-id']}&page={$i}'>{$i}</a></li>";
                            }

                            if($pageCount > $curren_page){
                                echo "<li><a href='{$_SERVER['PHP_SELF']}?aut-id={$_GET['aut-id']}&page=".($curren_page + 1)."'>Next</a></li>";
                            }
