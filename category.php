...
<?php
    ...
    include "config.php";
    $sql1 =  "SELECT * FROM category WHERE category_id = {$_GET['cat-id']}";
    ...
    if(mysqli_num_rows($result1) > 0){
        ...
        $sql =  "SELECT 
                ...
                WHERE post.category = '{$_GET['cat-id']}'
            ...
            <div class="col-md-8">
                ...
                <div class="post-container">
                  ...
                  <?php 
                        ...
                        <?php
                        $sqlPagQuery = "SELECT * FROM post WHERE post.category = '{$_GET['cat-id']}'";
                        ...
                        if(mysqli_num_rows($result2) > 0){
                            ...
                            if($curren_page > 1){
                                echo "<li><a href='{$_SERVER['PHP_SELF']}?cat-id={$_GET['cat-id']}&page=".($curren_page - 1)."'>Prev</a></li>";
                            }

                            for($i = 1; $i <= $pageCount; $i++){
                                $active_class = ($i == $curren_page) ? "active" : "";
                                echo "<li class='{$active_class}'><a href='{$_SERVER['PHP_SELF']}?cat-id={$_GET['cat-id']}&page={$i}'>{$i}</a></li>";
                            }

                            if($pageCount > $curren_page){
                                echo "<li><a href='{$_SERVER['PHP_SELF']}?cat-id={$_GET['cat-id']}&page=".($curren_page + 1)."'>Next</a></li>";
                            }
